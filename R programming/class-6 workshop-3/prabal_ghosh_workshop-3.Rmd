...
### 1.Read in the “cancer_stats.csv” file into a variable called cancer then answer the following questions.
...
festival_data_clean_data_join <- left_join(festival_data_drop, festival_data_clean_data, by ='person')
festival_data_clean_data_join
```
```{r}
# festival_data_clean_data_join <- left_join(festival_data, festival_data_clean_data, by ='person')

festival_data_clean_data_join %>%ggplot()+geom_jitter(aes(x=gender, y= cleanliness,col=gender))+ facet_wrap(~ day+attendance, scales = "free")+
  labs(
